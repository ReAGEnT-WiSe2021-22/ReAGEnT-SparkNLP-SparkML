...
case class TweetSourceParser() extends Converter {
  ...
  def rowToParser(row: Row): TweetCount = {
    ...
    val source = row.getValuesMap(Seq("source")).asInstanceOf[Map[String, Any]].values.toList.head.asInstanceOf[String]
